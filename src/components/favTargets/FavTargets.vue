<template>
  <div>
    ...
    <button
      ...
      class="group relative p-4 bg-gradient-to-r from-purple-600 to-purple-500 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105"
    >
  ...
  <div>
    ...
    >
      <div class="bg-gray-800 text-white p-4 m-8 rounded-lg max-w-2xl max-h-[80vh] overflow-y-auto">
        ...
        <div>
          <h2 class="text-xl font-bold mb-4">{{ $t('components.fav_target.titel') }}</h2>

          <div v-if="favTargetsStore.favoriteTargets.length">
            <table
              class="w-full text-sm text-left border border-gray-600 rounded-lg overflow-hidden"
            >
          ...
          <div v-else class="text-center py-8 text-gray-400">
            <HeartIcon class="w-12 h-12 mx-auto mb-4 text-gray-600" />
            <p>{{ $t('components.fav_target.no_fav') }}</p>
          </div>
        </div>
      </div>
    </div>
